...
async def run_migrations():
    ...
    async with pool.acquire() as conn:
        ...
        # Privacy-safe cohort learning: aggregate what worked for similar
        # starting-state/goal clusters without exposing individual user paths.
        await conn.execute("""
        ...
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_ioo_cohort_edge_stats_lookup
            ON ioo_cohort_edge_stats(cohort_key, to_node_id, confidence DESC)
        """)
        await conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_ioo_cohort_edge_stats_global
            ON ioo_cohort_edge_stats(to_node_id, confidence DESC)
        """)
