"""
Aura Health & Meta Routes

GET  /api/aura/health/dashboard  — full system status dashboard
GET  /api/aura/meta/report       — Aura's self-improvement meta report (cached)
POST /api/aura/meta/report       — trigger fresh MetaAgent analysis
POST /api/aura/report/daily      — generate and send daily Telegram report
...
router = APIRouter(tags=["aura-ops"])


# ---------------------------------------------------------------------------
# GET /api/aura/health/dashboard
# ---------------------------------------------------------------------------

@router.get("/api/aura/health/dashboard")
async def aura_health_dashboard() -> Dict[str, Any]:
    ...
    try:
        if r and redis_ok:
            world_ts = await r.get("aura:world:last_sync")
            if world_ts:
...
# ---------------------------------------------------------------------------
# GET /api/aura/meta/report — return cached MetaAgent report
# ---------------------------------------------------------------------------

@router.get("/api/aura/meta/report")
async def get_meta_report() -> Dict[str, Any]:
    """
    Return Aura's most recent self-improvement report from Redis cache.
...
@router.post("/api/aura/meta/report")
...
# ---------------------------------------------------------------------------
# POST /api/aura/report/daily — generate and send Telegram daily report
# ---------------------------------------------------------------------------

@router.post("/api/aura/report/daily")
async def send_daily_report() -> Dict[str, Any]:
    """
    Generate a daily system summary and send it to Avi via Telegram.
    """
    from aura.agents.reporter_agent import get_reporter
