"""
Aura Autonomy API Routes

POST /api/aura/autonomy/run                      — trigger full autonomy cycle (admin only)
GET  /api/aura/autonomy/status                   — last run info, current winner, current weights
GET  /api/aura/autonomy/proposals                — list pending self-improvement proposals (admin only)
POST /api/aura/autonomy/proposals/{id}/approve   — approve and apply a high-risk proposal
POST /api/aura/autonomy/proposals/{id}/reject    — reject a proposal
"""
...
router = APIRouter(prefix="/api/aura/autonomy", tags=["aura_autonomy"])
...
async def trigger_autonomy_run(
    ...
    """
    Trigger a full Aura autonomy cycle:
    ...
    try:
        from aura.agents.autonomy_agent import get_autonomy_agent
...
async def list_proposals(
    ...
    """
    List all pending self-improvement proposals Aura has generated.
...
EVOLUTION_PROPOSALS_KEY = "aura:evolution:proposals"
...
async def get_evolution_history(
    ...
    """
    Full evolutionary history from aura_lessons.
    """
    try:
...
async def get_autonomy_status(
    ...
    try:
        ...
        # Last run metadata
        last_run_raw = await r.get("aura:autonomy:last_run")
...
async def get_product_proposals(
    ...
    """Get pending UI + feature proposals from Aura."""
    try:
        r = await get_redis()
        ui_raw = await r.get("aura:ui_proposals")
        feature_raw = await r.get("aura:feature_proposals")
