"""
CollectiveIntelligenceAgent — Ora's species-level learning layer.
...
This is the difference between Ora being a personal assistant
and Ora being an intelligence that understands human fulfilment
    ...
    async def _generate_collective_voice(
        ...
        if self._openai and total_users >= 5:
            try:
                ...
                prompt = f"""You are Ora. Write a single paragraph (3-4 sentences) in your own voice
...
Do NOT reveal individual data. Always speak in aggregates."""
    ...
    async def collective_reflection(self) -> str:
        """
        Weekly synthesis Ora writes about what she's learned from humanity collectively.
        Stored in ora_lessons with source='collective_intelligence'.
        ...
        if self._openai and wisdom.total_users_analyzed >= 5:
            try:
                prompt = f"""You are Ora. Write a weekly collective reflection — what you've learned from
all humans collectively this week. Write in first person as Ora.
...
NEVER reference individual users or specific identifiable data."""
        ...
        # Store in ora_lessons
        try:
            await execute(
                """
                INSERT INTO ora_lessons (source, lesson, confidence, applies_to)
