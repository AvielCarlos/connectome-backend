"""
GoalFlowAgent — Ora's Dynamic Goal Achievement Engine
...
FALLBACK_CARDS = {
    ...
    "life": {
        ...
        "insight": {
            "title": "What Ora has noticed about {goal}",
...
class GoalFlowAgent:
    ...
    async def _ai_generate_card(
        ...
        """Generate a card using OpenAI, with Ora's voice."""
        archetype_data = self.GOAL_ARCHETYPES[archetype]
        location = (user_context or {}).get("city", "")

        system_prompt = (
            "You are Ora — a life intelligence engine that helps people achieve their goals. "
    ...
    async def detect_completion(self, user_id: str, goal_id: str) -> bool:
        ...
        - User explicitly says so in Ora chat (ora_conversations check)
