...
class CMOAgent(BaseExecutiveAgent):
    ...
    async def recommend(self) -> List[str]:
        ...
        if data["viral_coefficient_est"] < 0.05:
            recs.append("Viral K is low — add in-app sharing moments (share goal milestone, share Ora insight)")
    ...
    async def generate_campaign(self, channel: str = "organic") -> str:
        ...
        experiments = [
            "Themed week: 'Clarity Week' — daily Ora prompts about mental clarity shared on Twitter",
            "Challenge: '7-day goal sprint' — users share progress daily with #OraGoals hashtag",
            "Creator collab: DM 5 productivity YouTubers with a personalized Ora demo",
            "Feature spotlight: Thread on X about Ora's AI goal coach — real user success stories",
            ...
            "Case study: Write up one power user's transformation using Ora → post as blog + thread",
        ]
