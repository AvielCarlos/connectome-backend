"""
Ora Knowledge Graph Agent — Semantic graph of everything Ora has learned.

The knowledge graph is Ora's model-independent intelligence:
  ...
  - Survives model changes — Ora's knowledge travels with her
  - Queryable at inference time to augment system prompt

Schema: ora_knowledge_graph table (see core/database.py for migration)
Redis cache: ora:knowledge_graph (JSON, refreshed daily)
...
GRAPH_REDIS_KEY = "ora:knowledge_graph"
GRAPH_CACHE_TTL = 25 * 3600  # 25h — refreshed daily

NODE_TYPES = {
    "concept": "General concept or framework",
    "user_archetype": "Type of user Ora commonly works with",
...
class KnowledgeGraphAgent:
    """
    Builds and maintains Ora's semantic knowledge graph.
    ...
    async def build_graph_from_lessons(self) -> dict:
        """
        Process recent ora_lessons and extract nodes + edges.
        ...
        lessons = await db_fetch("""
            ...
            FROM ora_lessons ol
            WHERE NOT EXISTS (
                SELECT 1 FROM ora_knowledge_graph okg
    ...
    async def _upsert_elements(
        ...
        for node in nodes:
            ...
            existing = await fetchrow(
                "SELECT id, evidence_count, connections FROM ora_knowledge_graph WHERE label = $1",
                label,
            )
            ...
            if existing:
                ...
                await db_exec(
                    "UPDATE ora_knowledge_graph SET evidence_count = evidence_count + 1, updated_at = NOW() WHERE id = $1",
                    existing["id"],
                )
            else:
                # Create new node
                ...
                await db_exec(
                    """
                    INSERT INTO ora_knowledge_graph
        ...
        for edge in edges:
            ...
            if not from_id:
                # Try to find in DB
                row = await fetchrow(
                    "SELECT id FROM ora_knowledge_graph WHERE label = $1", from_label
                )
            ...
            if not to_id:
                row = await fetchrow(
                    "SELECT id FROM ora_knowledge_graph WHERE label = $1", to_label
                )
            ...
            if from_id and to_id:
                ...
                from_row = await fetchrow(
                    "SELECT connections FROM ora_knowledge_graph WHERE id = $1", from_id
                )
                ...
                await db_exec(
                    "UPDATE ora_knowledge_graph SET connections = $1::jsonb, updated_at = NOW() WHERE id = $2",
    ...
    async def query_graph(self, context: str, max_results: int = 5) -> List[dict]:
        ...
        Used to augment Ora's system prompt.
        """
    ...
    async def query_graph_for_prompt(self, context: str) -> str:
        """
        Returns a formatted string of relevant knowledge for Ora's system prompt.
        """
        nodes = await self.query_graph(context, max_results=3)
        if not nodes:
            return ""

        lines = ["Relevant knowledge from Ora's experience:"]
        for node in nodes:
    ...
    async def export_graph(self) -> dict:
        ...
        from core.database import fetch as db_fetch
        rows = await db_fetch(
            "SELECT * FROM ora_knowledge_graph ORDER BY evidence_count DESC"
        )
