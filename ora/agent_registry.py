"""
Ora Agent Registry — Living Agent Population

Ora's dynamic agent roster. Instead of hardcoded agent lists, all agents
(builtin + evolved) live here, persisted in Redis as ora:agent_registry.

Evolution events (partition, merge, spawn, cannibalize, retire) are also
logged to ora_lessons for long-term learning.
"""
...
REGISTRY_REDIS_KEY = "ora:agent_registry"
REGISTRY_TTL = 90 * 24 * 3600  # 90 days


class AgentRegistry:
    """
    Ora's living agent population. Stored in Redis + DB.
    ...
    Each agent dict has:
      ...
      module_path   — e.g. "ora.agents.spawned_goalcoach_agent" for evolved
    ...
    BUILTIN_AGENTS: List[Dict[str, Any]] = [
        {
            ...
            "module_path": "ora.agents.discovery",
        ...
        },
        {
            ...
            "module_path": "ora.agents.coaching",
            "fitness_at_creation": None,
            "created_at": "2024-01-01T00:00:00Z",
            "retired_at": None,
            "retire_reason": None,
        },
        {
            ...
            "module_path": "ora.agents.recommendation",
            "fitness_at_creation": None,
            "created_at": "2024-01-01T00:00:00Z",
            "retired_at": None,
            "retire_reason": None,
        },
        {
            ...
            "module_path": "ora.agents.world_agent",
            "fitness_at_creation": None,
            "created_at": "2024-01-01T00:00:00Z",
            "retired_at": None,
            "retire_reason": None,
        },
        {
            ...
            "module_path": "ora.agents.enlightenment",
            "fitness_at_creation": None,
            "created_at": "2024-01-01T00:00:00Z",
            "retired_at": None,
            "retire_reason": None,
        },
        {
            ...
            "module_path": "ora.agents.collective_intelligence",
            "fitness_at_creation": None,
            "created_at": "2024-01-01T00:00:00Z",
            "retired_at": None,
            "retire_reason": None,
    ...
    async def save(self, agents: List[Dict[str, Any]]) -> None:
        """Persist agent registry to Redis: ora:agent_registry."""
        try:
            from core.redis_client import get_redis
            r = await get_redis()
    ...
    async def retire_agent(self, name: str, reason: str) -> None:
        """Mark agent as retired, log to ora_lessons."""
        agents = await self.load()
        for agent in agents:
            if agent["name"] == name:
