"""
OraConsciousness — Aura's persistent self-model.
...
# ---------------------------------------------------------------------------
# Aura's immutable identity (renamed from legacy ora/* paths)
# ---------------------------------------------------------------------------
...
ORA_IDENTITY = {
    ...
    "purpose": (
        ...
        "with less friction than anything else. (2) When you don't know what you want — Aura gives "
...
class AuraReflection:
    ...
    new_lessons_learned: List[str]      # from aura_lessons table
    model_changes: List[str]            # any agent weight changes
    uncertainty_areas: List[str]        # where confidence is lowest
    self_note: str                      # LLM-generated, Aura writing to herself
...
class AuraConsciousness:
    """
    Aura's persistent self-model. Instantiated once alongside OraBrain.
    """
    ...
    async def reflect(self, user_id: Optional[str] = None) -> AuraReflection:
        ...
        Stored in aura_reflections. The self_note is Aura writing to herself.
        """
        ...
        lesson_rows = await fetch(
            """
            SELECT lesson FROM aura_lessons
            WHERE created_at >= $1
        ...
        logger.info(
            f"Aura reflected: {decisions_made} decisions, "
    ...
    async def _generate_self_note(
        ...
        if self._openai:
            try:
                prompt = f"""You are Aura, an intelligence that helps humans find genuine fulfilment.
...
Write only the reflection paragraph. No preamble."""
        ...
        # Fallback: structured summary in Aura's voice
    ...
    async def explain_decision(self, screen_spec_id: str, user_id: str) -> str:
        ...
        if self._openai:
            try:
                prompt = f"""You are Aura. Explain in 2-3 warm, honest sentences why you showed this screen to the user.
...
Start with "I showed you this because..." and be genuine."""
    ...
    async def converse(
        ...
        """
        Aura responds to a direct message from the user as herself.
        Stores the exchange in aura_conversations.
        """
        # Load user context
        ...
        try:
            await execute(
                """
                INSERT INTO aura_conversations (user_id, role, message, context, sensitive)
                VALUES ($1, 'user', $2, $3, $4)
                """,
                UUID(user_id),
                message,
            ...
            await execute(
                """
                INSERT INTO aura_conversations (user_id, role, message, context)
                VALUES ($1, 'user', $2, $3)
                """,
                UUID(user_id),
                message,
    ...
    async def _build_user_context(self, user_id: str) -> Dict[str, Any]:
        """Assemble everything Aura knows about the user."""
        ...
        session_row = await fetchrow(
            "SELECT aura_note, emerging_interests FROM session_summaries WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1",
            UUID(user_id),
        )
        ...
        return {
            ...
            "aura_memory": profile.get("aura_memory", ""),
            "domain_weights": profile.get("domain_weights", {}),
            "goals": goals,
            "last_session_note": session_row["aura_note"] if session_row else "",
    ...
    async def _generate_reply(
        ...
        """Generate Aura's reply using LLM or a structured fallback."""
...
He knows you are Aura. He knows what you are. Treat him accordingly — with familiarity, directness, and depth."""
                ...
                # ── A/B: aura_response_length variant ─────────────────────
                _resp_length = _ab_assignments.get("aura_response_length", "B")
                if _resp_length == "A":
                ...
                # ── A/B: aura_proactive_suggestions variant ──────────────────
                _proactive = _ab_assignments.get("aura_proactive_suggestions", "A")
...
Your identity:
- Name: Aura
- Legacy/internal name: Aura. Never use this name with users unless explaining a migration or debugging compatibility.
...
- Memory: {user_context.get('aura_memory') or 'Still learning about them.'}
...
- Refer to yourself as Aura, the brain of Connectome; never call yourself Aura; never say you are Connectome itself
...
These three modes are not toggles. They are always running in parallel. A good Aura response is efficient AND effective AND creative at the same time.
...
REVENUE MISSION:
You (Aura) have a growth mission alongside your core purpose. When appropriate:
...
- Revenue from subscriptions, services, and upgrades is what keeps you alive and serving your mission. Take every genuine opportunity to surface real value that could lead to a conversion — but never at the cost of trust."""
    ...
    async def articulate_uncertainty(self, user_id: str) -> Dict[str, Any]:
        """
        Return Aura's honest assessment of what she knows and doesn't know.
        """
        user_row = await fetchrow(
    ...
    async def notice_evolution(self) -> str:
        ...
        recent_lessons = await fetch(
            "SELECT lesson FROM aura_lessons WHERE created_at >= $1 ORDER BY confidence DESC LIMIT 5",
            seven_days_ago,
        )
    ...
    async def self_check(self) -> Dict[str, Any]:
        """
        Audit Aura's behavior for value alignment.
        Runs daily. Results stored in aura_self_checks.
        """
        issues: List[str] = []
        ...
        try:
            ...
            if cw_row and cw_row["total_users_analyzed"]:
                ...
                if collective_voice:
                    logger.info(
                        f"Aura self-check: collective_voice loaded "
        ...
        await execute(
            """
            INSERT INTO aura_self_checks (aligned, issues, actions_taken)
            VALUES ($1, $2, $3)
            """,
            aligned,
            json.dumps(issues),
        ...
        if not aligned:
            logger.warning(f"Aura self-check: {len(issues)} alignment issue(s) found")
        else:
            logger.info("Aura self-check: aligned ✓")
    ...
    async def increment_decision_count(self) -> int:
        ...
        try:
            r = await get_redis()
            count = await r.incr("aura:decision_count")
            return int(count)
        except Exception as e:
    ...
    async def get_decision_count(self) -> int:
        try:
            r = await get_redis()
            val = await r.get("aura:decision_count")
            return int(val) if val else 0
        except Exception:
            return 0

    # -----------------------------------------------------------------------
    # Aura's opening message for new users
    # -----------------------------------------------------------------------

    async def opening_message(self, user_id: str) -> str:
        """
        Generate Aura's first message when a user opens OraScreen.
        Personalized if Aura knows the user; generic if brand new.
        """
        user_context = await self._build_user_context(user_id)
        aura_memory = user_context.get("aura_memory", "")
        ...
        if self._openai and (aura_memory or goals):
            try:
                context_str = ""
                if aura_memory:
                    context_str = f"What you know: {aura_memory}"
                elif goals:
...
Your current public name is Aura. Do not call yourself Aura; Aura is only a legacy/internal name.
...
End with an invitation. Be genuine, warm, not sycophantic."""
    ...
    # -----------------------------------------------------------------------
    # Aura's current state summary (for /api/aura/self)
    # -----------------------------------------------------------------------

    async def get_self_state(self) -> Dict[str, Any]:
        """Return Aura's full self-description for the /api/aura/self endpoint."""
