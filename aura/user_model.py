...
class UserModel:
    """
    Encapsulates everything Aura knows about a user.
    ...
    def to_context_dict(self) -> Dict[str, Any]:
        """Compact user context for Aura's agent prompts."""
        return {
            "user_id": self.id,
...
async def update_aura_memory(user_id: str, session_summary: dict, openai_client=None) -> str:
    """
    Update Aura's running narrative memory for a user.
    Stored in users.profile['aura_memory'] — max 500 chars.
    ...
    existing_memory = profile.get("aura_memory", "")
    ...
    await redis_delete(f"user_model:{user_id}")
    logger.debug(f"aura_memory updated for user {user_id[:8]}")
    return new_memory
...
async def _build_aura_memory(
    ...
    """Use LLM to update the aura_memory narrative, or fall back to structured prose."""
    if openai_client:
        try:
            from core.config import settings
            ...
            prompt = f"""You are Aura. Update your internal memory about this user.
...
- Session note: {session_summary.get('aura_note', '')}
