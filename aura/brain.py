"""
Aura Brain — Supreme Intelligence Layer

Aura is the JARVIS of human life: simultaneously the world's best recommender,
...
Aura's mission: help each person find, achieve, and experience everything
...
from aura.ab_testing import assign_user_variant, record_test_result
from aura.agents.discovery import DiscoveryAgent
from aura.agents.coaching import CoachingAgent
from aura.agents.recommendation import RecommendationAgent
from aura.agents.ui_generator import UIGeneratorAgent
from aura.agents.feedback_analyst import FeedbackAnalystAgent
from aura.agents.world_agent import WorldAgent
from aura.agents.feedback_experimenter import FeedbackExperimenter
from aura.agents.enlightenment import EnlightenmentAgent
from aura.agents.collective_intelligence import CollectiveIntelligenceAgent
from aura.agents.discovery_interview import DiscoveryInterviewAgent
from aura.agents.ui_ab_testing import UIABTestingAgent
from aura.agents.explore import ExploreAgent
from aura.agents.feature_lab import AuraFeatureLabAgent
from aura.agents.dao_agent import DaoAgent
from aura.agents.world_discovery_agent import WorldDiscoveryAgent
from aura.agents.drive_agent import DriveAgent
from aura.consciousness import AuraConsciousness
from aura.content_quality import content_quality_check
from aura.agents.context_agent import ContextAgent
from aura.agent_registry import AgentRegistry
from aura.agents.goal_flow_agent import GoalFlowAgent, get_goal_flow_agent
from aura.agents.recommendation_engine import RecommendationEngine, get_recommendation_engine

# Module-level alias so OraBrain code can call _content_quality_check(spec)
_content_quality_check = content_quality_check

# Aura's optimization priority — hard constraint, never overridden
ORA_GOALS = {
...
class AuraBrain:
    """
    Aura's central brain. One instance per application lifecycle.
    ...
    def __init__(self):
        ...
        if settings.has_openai:
            try:
                ...
                logger.info("Aura: OpenAI client initialized")
            except Exception as e:
                logger.warning(f"Aura: Could not initialize OpenAI: {e}")
        else:
            logger.info("Aura: No OPENAI_API_KEY — running in mock mode")
        ...
        # Drive indexer — Aura's long-term memory from personal writing
        self.drive_agent = DriveAgent(self._openai)

        # Aura's self-awareness layer
        ...
        # WebSpawn — Aura's surface creation engine (explorer/sovereign only)
    ...
    async def reload_weights(self) -> None:
        """
        Check Redis key `aura:agent_weights` and override _base_weights if found.
    ...
    async def get_screen(
        ...
        if should_interview:
            ...
            logger.info(
                f"Aura: injecting discovery interview at screen #{screens_today} "
        ...
        if lessons:
            user_context["aura_lessons"] = lessons

        logger.info(
            f"Aura→ user={user_id[:8]} agent={agent_name} variant={variant} "
        ...
        if _use_vector:
            ...
            if _v_cards:
                ...
                exploit = True
                logger.info(f"Aura: vector similarity exploit → {len(_v_cards)} candidates for user={user_id[:8]}")
            else:
        ...
        #   3. Aura's daily mood is exploratory
        ...
        for _quality_attempt in range(3):
            if _content_quality_check(spec_dict):
                break
            logger.info(f"Aura: content quality fail attempt {_quality_attempt + 1} — regenerating")
            try:
                spec_dict = await agent_fn(user_context, variant=variant)
                spec_dict["domain"] = domain
                spec_dict["screen_id"] = screen_id
        ...
        if _goal_injection_active and _goal_dice < 0.20:
            try:
                ...
                if goal_card:
                    ...
                    logger.info(f"Aura: goal flow injection for user={user_id[:8]} screen#{screens_today}")
            except Exception as _gfe:
                logger.debug(f"Aura: goal flow injection failed: {_gfe}")
                    ...
                    for _rec_id in _rec_ids:
                        try:
                            ...
                            if _rec_row and _rec_row["spec"]:
                                ...
                                logger.debug(f"Aura: vector rec served card {_rec_id[:8]} for user={user_id[:8]}")
                                break
                        except Exception:
                            continue
            except Exception as _vre:
                logger.debug(f"Aura: vector recommendation failed: {_vre}")
        ...
        try:
            ...
            if decision_count > 0 and decision_count % AuraConsciousness.DECISIONS_PER_REFLECTION == 0:
                ...
                logger.info(f"Aura: triggered reflection at {decision_count} decisions")
    ...
    async def _get_exploit_ratio(self) -> float:
        """Read aura:exploit_ratio from Redis (default 0.7)."""
        try:
            from core.redis_client import get_redis
            r = await get_redis()
            val = await r.get("aura:exploit_ratio")
            if val:
    ...
    async def _maybe_inject_world_card(
        ...
        Triggers when:
          ...
          3. Aura's daily mood is exploratory (Redis flag 'aura_mood_exploratory')
        ...
        try:
            ...
            # Check Aura's daily exploratory mood toggle
            aura_exploratory = False
            try:
                from core.redis_client import get_redis
                r = await get_redis()
                mood_flag = await r.get('aura_mood_exploratory')
            ...
            logger.info(
                f"Aura: injecting world-aware card "
                f"(no_goals={not has_goals}, roll={serendipity_roll:.2f}, "
                f"exploratory={aura_exploratory})"
            )
    ...
    async def _select_agent(
        ...
        # Redis key `aura:exploit_ratio` (default 0.7) to decide serve direction.
        # -----------------------------------------------------------------------
        try:
            ...
            if _twitter_signals_raw:
                ...
                if random.random() < _exploit_ratio:
                    ...
                    logger.info(
                        f"Aura: Twitter exploit path (ratio={_exploit_ratio:.2f}) for user={user_model.id[:8]}"
                    )
                    ...
                    logger.info(
                        f"Aura: Twitter explore path (ratio={_exploit_ratio:.2f}) for user={user_model.id[:8]}"
                    )
        ...
        try:
            if await self.collective.is_suppressed(name, domain if domain else None):
                logger.info(
                    f"Aura: {name}/{domain} is collectively suppressed — switching to DiscoveryAgent"
                )
                return "DiscoveryAgent", self.discovery.generate_screen, False
        ...
        if name == "WorldAgent":
            ...
            if consecutive_world:
                logger.info("Aura: blocking 3rd consecutive WorldAgent screen — switching to DiscoveryAgent")
                return "DiscoveryAgent", self.discovery.generate_screen, False
        ...
        if name == "WorldAgent":
            ...
            if world_ratio >= cap:
                logger.info(f"Aura: WorldAgent at cap ({world_ratio:.0%} >= {cap:.0%}) — switching to DiscoveryAgent")
                return "DiscoveryAgent", self.discovery.generate_screen, False
    ...
    async def _cold_start_agent(
        ...
        logger.info(f"Aura: cold-start selection → {name} (slot {idx}/{len(all_agents)})")
        return name, fn, False
    ...
    async def _ensure_novelty(
        ...
        if len(set(last_5_domains)) == 1 and last_5_domains[0] == domain:
            ...
            logger.info(f"Aura: novelty override — switching domain to {domain} (last 5 were same)")
        ...
        if len(set(last_3_agents)) == 1 and last_3_agents[0] == agent_name:
            ...
            if agent_name in alt_agent_map:
                ...
                logger.info(f"Aura: novelty override — switching agent to {agent_name} (last 3 were same)")
        ...
        try:
            ...
            if novelty < 0.4:
                logger.info(f"Aura: novelty score {novelty:.2f} < 0.4 — injecting exploration")
    ...
    # -----------------------------------------------------------------------
    # Aura Lessons — growing knowledge base
    # -----------------------------------------------------------------------
    ...
    async def load_relevant_lessons(
        ...
        """
        Load the most recent applicable lessons from aura_lessons.
    ...
    async def _session_summary_llm(
        ...
        prompt = f"""You are Aura, analyzing a user's session to build a rich internal model.
...
{{
  ...
  "avoid_topics": ["topic1"],
  "aura_note": "brief internal note about user's session pattern",
...
Return ONLY valid JSON."""
    ...
    def _session_summary_mock(
        ...
        return {
            ...
            "aura_note": f"{tone} Avg rating {avg_rating:.1f}/5 across {screens_shown} screens.",
    ...
    async def schedule_reengagement(
        ...
        # Random delay: 2-6 hours (Aura will refine this using return_rate_signal)
    ...
    async def _generate_reengagement_message(
        ...
        if self._openai and settings.has_openai:
            try:
                aura_note = session_summary.get("aura_note", "")
                prompt = (
                    f"Write a short, warm push notification (max 120 chars) to re-engage a user "
                    f"who is working toward: '{goal_title}'. "
                    f"Context from their session: {aura_note[:100]}. "
                    f"Mention Aura has a new idea for their next step. End with a question."
                )
                response = await self._openai.chat.completions.create(
                    model="gpt-4o",
                    messages=[{"role": "user", "content": prompt}],
        ...
        return (
            ...
            f"Aura has a new idea for your next step. Ready?"
        )
