"""
Growth billing helpers for Aura/Connectome revenue experiments.
...
API_ACCESS_PLANS: Dict[str, Dict[str, Any]] = {
    "developer": {
        "name": "Aura Developer API — 10k calls",
        "unit_amount": 2900,
        "included_calls": 10_000,
        "interval": "month",
    },
    "scale": {
        "name": "Aura Developer API — 100k calls",
        "unit_amount": 9900,
...
ORA_SESSION_TYPES: Dict[str, Dict[str, Any]] = {
    "clarity": {
        "name": "Aura Clarity Session",
        "description": "One premium 1:1 coaching session with Aura's human-flourishing workflow.",
        "unit_amount": 4900,
    },
    "deep_work": {
        "name": "Aura Deep Work Session",
...
async def create_corporate_plan_checkout(org_name: str, seats: int, contact_email: str) -> Dict[str, Any]:
    ...
    data: Dict[str, Any] = {
        ...
        "line_items[0][price_data][product_data][name]": "Aura Corporate Wellness Seat",
        "line_items[0][price_data][product_data][description]": (
            "Bulk Aura access for workplace clarity, fulfilment, and aligned action. Minimum 10 seats."
        ),
        "allow_promotion_codes": "true",
...
async def create_aura_session_payment(user_id: str, session_type: str) -> Dict[str, Any]:
    """Create a one-off Stripe Checkout Session for a premium Aura coaching session."""
    type_key = (session_type or "clarity").lower()
    if type_key not in ORA_SESSION_TYPES:
        raise ValueError(f"Unknown Aura session type: {session_type}")
    ...
    data: Dict[str, Any] = {
        ...
        "metadata[growth_stream]": "aura_session",
        "metadata[connectome_user_id]": str(user_id),
        "metadata[session_type]": type_key,
        "payment_intent_data[metadata][growth_stream]": "aura_session",
