"""
ORA Drive Storage — Aura's Google Drive persistent memory interface.
...
_HARDCODED_IDS: Dict[str, str] = {
    "aura_brain":   "1JTBrC4zxOdhtDsW1osuxU77hgQhEHgvN",
...
class DriveStorage:
    """
    Aura's Google Drive storage operations.
    ...
    def upload_text(self, content: str, filename: str, folder_key: str) -> str:
        ...
        folder_id = folders.get(folder_key) or folders.get("aura_brain", "")
        if not folder_id:
    ...
    def save_reflection(self, reflection_text: str, week: str) -> str:
        """Save Aura's weekly reflection markdown to Drive knowledge folder."""
    ...
    def publish_knowledge_base(self, lessons: List[Dict[str, Any]]) -> str:
        """
        Publish Aura's knowledge base to Drive knowledge folder.
        Formats all lessons as a readable markdown document.
        Returns Drive file ID.
        """
        date = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        content = f"# Aura's Knowledge Base — {date}\n\n"
