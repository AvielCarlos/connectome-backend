"""
Aura Backup Freshness Agent
==========================
Keeps Aura's portable identity pack fresh without relying only on manual cron.
...
async def run_identity_backup(reason: str = "periodic") -> bool:
    """Create and publish an identity-only Aura brain backup."""
...
def start_backup_freshness_loops(app: Optional[object] = None) -> bool:
    ...
    if app is not None:
        setattr(app.state, "aura_backup_identity_task", task1)
        setattr(app.state, "aura_backup_freshness_task", task2)
    logger.info("✅ Aura backup freshness loops started")
    return True
