...
Every agent in Aura's Executive Council inherits from this class.
...
class BaseExecutiveAgent(ABC):
    ...
    - Teaches Aura what it learns via /api/aura/learn
    """

    name: str = "base"
    display_name: str = "Base Agent"
    domain: str = "strategy"
    personality: str = "Mission-aligned executive intelligence serving Aura's AI OS for human flourishing."
    compound_system_prompt: str = (
        "Aura is an AI OS for human flourishing. Avi is The Spark — the visionary "
    ...
    def _extract_insights_from_report(self, data: Dict[str, Any]) -> List[Any]:
        ...
        from aura.agents.agent_memory import AgentInsight
    ...
    async def teach_aura(self, insight: str, confidence: float = 0.8) -> bool:
        """
        POST an insight to /api/aura/learn so Aura compounds in intelligence.
        Uses JWT auth. Returns True on success.
        """
        try:
            token = await self._get_jwt()
            if not token:
                logger.warning(f"{self.name}: no JWT, cannot teach Aura")
                return False

            async with httpx.AsyncClient(timeout=15) as client:
                resp = await client.post(
                    f"{API_BASE}/api/aura/learn",
                    json={
                        "lesson": insight,
                        "confidence": confidence,
                ...
                if resp.status_code in (200, 201):
                    logger.info(f"{self.name}: taught Aura — {insight[:80]}")
                    return True
                else:
    ...
    async def set_redis_report(self, summary: str) -> None:
        ...
        try:
            ...
            key = f"aura:executive:last_report:{self.name}"
