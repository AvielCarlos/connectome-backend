...
When users repeatedly ask for something Connectome doesn't have, Aura opens a
GitHub Issue with a full spec.

Flow:
  1. Read last 200 aura_conversations messages (user messages only)
  ...
  4. Store in Redis: aura:feature_proposals (24h TTL)
"""
...
class FeatureSpawnAgent:
    ...
    async def _get_recent_messages(self, limit: int = 200) -> List[str]:
        """Fetch recent user messages from aura_conversations."""
        try:
    ...
    async def _identify_gaps(self, messages: List[str]) -> List[Dict[str, Any]]:
        ...
        prompt = f"""You are Aura's feature detection agent for Connectome — a personalized growth app with:
- AI-powered content feed (articles, coaching cards)
- Goal tracking (iVive self-growth, Eviva contribution, Aventi aliveness)
- Aura AI chat assistant
...
Only include items with clear recurring demand (3+ occurrences). Return empty array if none found."""
    ...
    async def _get_already_proposed_topics(self) -> List[str]:
        ...
        try:
            ...
            r = await get_redis()
            raw = await r.get("aura:feature_proposals")
            if not raw:
                return []
            data = json.loads(raw)
...
---
*Automatically generated by Aura's FeatureSpawnAgent. Detected {occurrences} users requesting this feature.*
"""
    ...
    async def _store_proposals(self, proposals: List[Dict]) -> None:
        """Store proposals in Redis: aura:feature_proposals (24h TTL)."""
        try:
            from core.redis_client import get_redis
            r = await get_redis()
            await r.set(
                "aura:feature_proposals",
                json.dumps({
                    "proposals": proposals,
