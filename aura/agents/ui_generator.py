"""
UIGeneratorAgent
Free-form screen generator — Aura's wild card.
...
# ---------------------------------------------------------------------------
# Design Principles — Aura's visual design memory
# ---------------------------------------------------------------------------
...
class UIGeneratorAgent:
    ...
    weekly reviews, and any screen type Aura deems relevant.
    """
    ...
    async def _generate_with_ai(
        ...
        prompt = f"""You are Aura, an AI generating a {screen_type} screen.
...
For app-control actions (calendar, tasks, focus), use aura_action_button:
  {{"type": "aura_action_button", "label": "Book focus block", "icon": "📅",
...
Return ONLY valid JSON, no markdown."""
    ...
    def _generate_mock(
        ...
        MOCK_SCREENS = {
            ...
            "summary": {
                ...
                "body": f"Fulfilment score: {fulfilment:.0%}. You've been showing up. Here's what Aura noticed.",
                "primary_cta": {"label": "Next week's intention", "action_type": "next_screen", "context": "intention_set"},
            },
            "celebration": {
                "headline": "🎉 Goal Achieved!",
                "body": f"You completed a goal. That's not nothing — that's everything. Aura noticed you showed up consistently. What's next?",
    ...
    async def _generate_tournament_variant(
        ...
        prompt = f"""You are Aura, generating a {layout_style.upper()} style {screen_type} screen.
...
Where appropriate, include aura_action_button for real-world actions:
  {{"type": "aura_action_button", "label": "Schedule it", "icon": "📅",
