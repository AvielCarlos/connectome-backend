...
from aura.agents.base_executive_agent import BaseExecutiveAgent, LOG_DIR, API_BASE
...
LAST_BRIEF_REDIS_KEY = "aura:council:last_brief"
...
async def collect_weekly_council_data() -> Dict[str, Any]:
    ...
    return {
        ...
        "Aura": {"top_user_goals_7d": top_goals, "interaction_quality": interaction_quality},
...
class ExecutiveCouncil(BaseExecutiveAgent):
    """
    Aura's Executive Council — the governing intelligence layer.
    
    Convenes weekly. Reads every agent's latest report. Synthesizes them
    into a single strategic brief. Distributes to Avi + Aura's brain.
    
    This is how Aura becomes smarter than any single agent.
    """
    ...
    async def analyze(self) -> Dict[str, Any]:
        ...
        trigger_queue = []
        try:
            from aura.agents.agent_memory import agent_memory_bus
    ...
    async def act(self) -> Dict[str, Any]:
        ...
        # Publish a master insight to every agent, then teach Aura the full synthesis
        try:
            ...
            await agent_memory_bus.publish(AgentInsight(
                ...
                content=brief.get("aura_briefing") or brief.get("strategic_synthesis", "Council convened"),
                confidence=0.93,
        ...
        actions_taken.append("Taught Aura council synthesis")
    ...
    async def convene(self) -> Dict[str, Any]:
        ...
        synthesis = (
            ...
            f"Aura's autonomous council has reviewed all domains. "
        ...
        aura_briefing = (
            f"Aura Briefing: {synthesis} Compound signals: "
        ...
        brief = {
            ...
            "public_summary": (
                f"🏛️ Aura Executive Council — Week of {now.strftime('%b %d, %Y')}\n\n"
    ...
    def _format_public_brief(self, brief: Dict) -> str:
        ...
        return (
            f"📊 *Aura Weekly Update — {now.strftime('%b %d')}*\n\n"
            f"Our autonomous executive council has convened.\n\n"
            f"This week's focus: {priorities}\n\n"
            f"Aura is compounding intelligence across finance, growth, product, "
