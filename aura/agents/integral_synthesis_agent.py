"""
Integral Synthesis Agent — Aura's four-quadrant knowledge synthesis layer.

Synthesises Connectome/Aura knowledge from the public website, web/app surfaces,
...
class IntegralSynthesisAgent:
    """Builds Aura's integral world-model across I / IT / WE / ITS quadrants."""
    ...
    async def read_app_content(self) -> Dict[str, List[Dict[str, Any]]]:
        data: Dict[str, List[Dict[str, Any]]] = {"screen_specs": [], "aura_surfaces": []}
        try:
            rows = await fetch(
                """
        ...
        try:
            ...
            data["aura_surfaces"] = [_row_to_dict(row) for row in rows]
        except Exception as exc:
            logger.debug("IntegralSynthesisAgent aura_surfaces read failed: %s", exc)
        return data
    ...
    async def read_drive_docs(self, max_docs: int = 12) -> List[Dict[str, str]]:
        ...
        files = await _run_json_command([
            "gog", "drive", "list", "--account", "aura.ai.intelligence@gmail.com", "--client", "aura"
        ])
        docs: List[Dict[str, str]] = []
        if not isinstance(files, list):
            return docs

        strategy_re = re.compile(r"(strategy|vision|mission|aura|connectome|ascension|dao|product)", re.I)
        ...
        for item in candidates[:max_docs]:
            ...
            content = await _run_text_command([
                "gog", "docs", "cat", doc_id, "--account", "aura.ai.intelligence@gmail.com", "--client", "aura"
            ])
            if content:
...
def _extract_claims(item: Any) -> Dict[str, str]:
    ...
    topics = {
        ...
        "product_name": ["connectome", "ido", "aura"],
        "aura_role": ["aura role", "heart of connectome", "ai at the heart", "multi-agent"],
...
def _conflict_severity(topic: str, source_a: str, source_b: str, claim_a: str, claim_b: str) -> str:
    high_topics = {"mission", "product_name", "aura_role"}
