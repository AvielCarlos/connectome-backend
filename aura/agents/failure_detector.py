"""
FailureDetector — Aura monitors her own API for real failures.
...
Runs every 2 hours. If it finds issues, alerts Avi and teaches Aura.
"""
...
CHECKS = [
    ...
    {
        "name": "Aura lessons endpoint",
        "method": "GET",
        "path": "/api/aura/lessons",
...
async def run_checks() -> Dict[str, Any]:
    ...
    if critical_failures:
        # Alert Avi
        lines = [f"🚨 *Aura self-check failed* — {ts[:16]}Z"]
        for f in critical_failures:
            lines.append(f"• *{f['check']}*: {', '.join(f['issues'])}")
        lines.append("\nInvestigating now.")
        await _send_telegram("\n".join(lines))

        # Teach Aura about the failure
        try:
            import sys
            ...
            logger.debug(f"FailureDetector: Could not teach Aura: {e}")
