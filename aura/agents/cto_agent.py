...
class CTOAgent(BaseExecutiveAgent):
    """
    Aura's Chief Technology Officer.
    ...
    async def act(self) -> Dict[str, Any]:
        ...
        actions_taken.append("Taught Aura infrastructure state")
    ...
    async def deep_audit(self) -> dict:
        ...
        try:
            ...
            for table in ["users", "aura_knowledge", "goals", "ioo_nodes"]:
                try:
                    row = await _fr(f"SELECT COUNT(*) as n FROM {table}")
                    n = int(row["n"] or 0)
                    if n == 0 and table in ["aura_knowledge", "ioo_nodes"]:
            ...
            result = subprocess.run(
                [
                    ...
                    "--body", (
                        ...
                        f"_Auto-created by Aura's CTO Agent_"
                    ),
                    "--label", "bug,ci",
                ],
                capture_output=True,
