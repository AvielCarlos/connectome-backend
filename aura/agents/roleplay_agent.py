...
Rocky.ai-inspired: Aura takes on the role of the other person in a scenario
...
SCENARIOS: Dict[str, Dict[str, Any]] = {
    "job_interview": {
        "label": "Job Interview",
        "aura_role": "a professional hiring manager",
    ...
    "difficult_conversation": {
        ...
        "aura_role": "the other person in a difficult real-life conversation",
        "user_role": "yourself",
        "system_context": (
    ...
    "sales_pitch": {
        "label": "Sales Pitch",
        "aura_role": "a skeptical potential customer",
    ...
    "first_date": {
        "label": "First Date",
        "aura_role": "a date at a coffee shop",
        "user_role": "yourself",
        "system_context": (
    ...
    "negotiation": {
        "label": "Negotiation",
        "aura_role": "the other party in a negotiation",
...
class RolePlayAgent:
    ...
    Session lifecycle:
      ...
      2. send_message()   — Aura responds in character (multiple rounds)
      3. end_session()    — Aura breaks character and gives coaching debrief
    """
    ...
    async def start_session(
        ...
        """
        Create a new role-play session. Returns session_id + Aura's opening line.
        """
        ...
        return {
            ...
            "aura_role": scenario_def["aura_role"],
            "opening": opening,
        }
    ...
    async def send_message(
        ...
        """
        User sends a message; Aura responds in character.
        """
        ...
        session["history"].append({"role": "aura", "content": response})
        await self._save_session(session_id, session)
    ...
    async def end_session(self, session_id: str) -> Dict[str, Any]:
        """
        End the role-play session. Aura breaks character and gives coaching feedback.
        """
        session = await self._load_session(session_id)
        if not session:
    ...
    async def _contextualise_opening(
        ...
        try:
            prompt = (
                f"You are playing the role of {scenario_def['aura_role']} in a simulation.\n"
    ...
    async def _generate_in_character_response(
        ...
        """Generate Aura's in-character response."""
        if not self._openai:
        ...
        for turn in history:
            role = "assistant" if turn["role"] == "aura" else "user"
    ...
    async def _generate_debrief(
        ...
        transcript_lines = []
        for turn in history:
            speaker = "Aura (in character)" if turn["role"] == "aura" else "You"
            transcript_lines.append(f"{speaker}: {turn['content']}")
        transcript = "\n".join(transcript_lines)

        prompt = f"""You are Aura, an AI life coach. You just finished a role-play simulation.

Scenario: {scenario_def['label']}
You played: {scenario_def['aura_role']}
