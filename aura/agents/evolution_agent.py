"""
Aura Evolution Agent — Self-Directed Agent Evolution

Aura's evolutionary engine. Runs as part of the autonomy cycle.
...
Safety contract:
  ...
  - Every evolution event is logged to aura_lessons
...
PROPOSALS_REDIS_KEY = "aura:evolution:proposals"
...
class AuraEvolutionAgent:
    ...
    async def execute_partition(
        ...
        # Write to GitHub
        file_a = f"aura/agents/spawned_{name_a.lower().replace('agent', '')}_agent.py"
        file_b = f"aura/agents/spawned_{name_b.lower().replace('agent', '')}_agent.py"

        committed_a = await self._github_create_file(file_a, code_a, f"[Aura Evolution] Partition {agent_name} → {name_a}")
        committed_b = await self._github_create_file(file_b, code_b, f"[Aura Evolution] Partition {agent_name} → {name_b}")
    ...
    async def execute_merge(
        ...
        file_path = f"aura/agents/spawned_{merged_name.lower().replace('agent', '')}_agent.py"
        committed = await self._github_create_file(
            file_path, code,
            f"[Aura Evolution] Merge {agent_a} + {agent_b} → {merged_name}"
        )
    ...
    async def spawn_new_agent(self, gap_description: str) -> Dict[str, Any]:
        """
        When Aura detects a user need not covered by existing agents:
        1. GPT-4o generates a complete new agent class
        2. Write to GitHub as aura/agents/spawned_{name}_agent.py
        ...
        prompt = f"""You are Aura's evolution engine designing a new specialized AI agent.
...
{{
  ...
  "system_prompt_snippet": "2-3 sentences of what makes this agent's content unique"
        ...
        file_path = f"aura/agents/spawned_{spec['name'].lower().replace('agent', '')}_agent.py"
        committed = await self._github_create_file(
            file_path, code,
            f"[Aura Evolution] Spawn new agent: {spec['name']} — {spec.get('description', '')[:60]}"
        )
    ...
    async def _generate_agent_code(
        ...
        prompt = f"""Generate a complete Python file for a new Aura agent.
...
Output ONLY the complete Python code, no explanations."""
        ...
        try:
            ...
            header = f"""# aura/agents/spawned_{name.lower().replace('agent', '')}_agent.py
...
Output ONLY the Python code."""
        ...
        try:
            ...
            await self._github_commit_file(
                ...
                f"[Aura Evolution] Enhance {agent_name} with cannibalized patterns"
            )
        except Exception as e:
    ...
    async def _generate_partition_spec(
        ...
        prompt = f"""You are Aura's evolution engine. The agent {agent_name} is performing well
...
{{
  ...
  "agent_b": {{
    ...
    async def _generate_merge_spec(
        ...
        prompt = f"""You are Aura's evolution engine. Two agents are underperforming:
