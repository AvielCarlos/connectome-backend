...
from aura.agents.ux_selection_agent import build_ux_selection_payload
...
def clarify_execution_requirements(node: Any, user_context: Any) -> list[str]:
    ...
    if n.step_type in {"physical", "hybrid"} and not _context_location(context) and not n.requires_location:
        questions.append("Where should Aura optimise this for — your current city/neighbourhood or somewhere else?")
    ...
    if missing_skills:
        questions.append(f"Do you already have these prerequisites: {', '.join(missing_skills)} — or should Aura include a prep path?")
