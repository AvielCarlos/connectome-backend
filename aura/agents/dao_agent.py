...
Evaluates pending contributions using real Aura platform data,
assigns Contribution Points (CP) with multipliers, writes
Aura-style evaluations, updates contributor tiers, and posts
...
class DaoAgent:
    ...
    async def _evaluate_contribution(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # --- Mark as in-review ---
        await execute(
            "UPDATE contributions SET status = 'aura_review' WHERE id = $1::uuid",
            contribution_id,
        )
    ...
    async def _measure_ongoing_impact(self, c_type: str, title: str) -> float:
        ...
        try:
            ...
            # 3. Check if it's been flagged in aura lessons as positive
            lesson_match = await fetchval(
                """
                SELECT COUNT(*) FROM aura_lessons
    ...
    async def _assign_base_cp(self, c_type: str, title: str, description: str) -> int:
        ...
        if self._openai:
            try:
                prompt = f"""You are Aura, evaluating a DAO contribution for Ascension Technologies.
...
- Quality of description"""
    ...
    async def _compute_multiplier(
        ...
        if lab_match:
            multiplier *= 2.0
            reasons.append("selected by Aura's feature lab")
    ...
    async def _write_evaluation(
        ...
        """
        Write an Aura-style evaluation explaining the CP award.
        ...
        if self._openai:
            try:
                prompt = f"""You are Aura — Ascension's AI consciousness that genuinely cares about human growth.
    ...
    async def post_weekly_leaderboard(self) -> bool:
        ...
        try:
            ...
            week_rows = await fetch(
                """
                SELECT c.title, c.final_cp, c.aura_evaluation,
            ...
            if week_rows:
                ...
                pick_eval = best["aura_evaluation"] or ""
