"""
ExperimentGeneratorAgent — The evolutionary intelligence layer of Aura's A/B system.
...
class ExperimentGeneratorAgent:
    ...
    async def _ask_aura(self, prompt: str) -> str:
        """Ask Aura to generate content via the chat API."""
        try:
            token = await self._get_token()
            async with httpx.AsyncClient(timeout=30) as client:
                r = await client.post(f"{API_URL}/api/aura/chat",
                    headers={"Authorization": f"Bearer {token}"},
                    json={"message": prompt})
                return r.json().get("reply", "")
        except Exception as e:
            logger.error(f"Aura API error: {e}")
            return ""
    ...
    async def _teach_aura(self, lesson: str):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=15) as client:
                await client.post(f"{API_URL}/api/aura/learn",
                    headers={"Authorization": f"Bearer {token}"},
...
NEW_EXPERIMENT: [page] | [experiment name] | [variant A] | [variant B] | [variant C]"""
    ...
    async def prune_losers(self):
        """Learn what doesn't work and teach Aura to avoid it."""
        losers = await fetch("""
        ...
        if loser_patterns:
            ...
            await redis.set('aura:ab:loser_patterns', json.dumps(loser_patterns[-50:]), ex=86400*30)
            await redis.aclose()
    ...
    async def _seed_nav_experiments(self):
        ...
        nav_experiments = {
            ...
            "nav_aura_prominence": {
                "page": "global", "metric": "aura_session_starts",
                "variants": {
                    "A": "Aura same size as other tabs",
                    "B": "Aura elevated FAB-style center button",
                    "C": "Aura replaces home — primary tab",
                    "D": "Aura tab pulses when she has something to say"
                }
            },
            "nav_item_count": {
                "page": "global", "metric": "section_discovery_rate",
                "variants": {
                    "A": "3 items: Feed, Aura, Profile",
                    "B": "4 items: Feed, Aura, Goals, Profile",
                    "C": "5 items: Feed, Aura, Goals, DAO, Profile",
                    "D": "2+overflow: Feed, Aura, ··· (more)"
                }
            },
