"""
FeedbackExperimenter — Aura's Meta-Learning Agent
Autonomously designs, deploys, and evaluates new feedback collection methods.
Feedback mechanisms are themselves A/B tested — Aura learns which ways of asking
produce the most signal. Results are stored as lessons in aura_lessons, feeding
...
FEEDBACK_MECHANISM_TYPES: Dict[str, str] = {
    ...
    "micro_poll":         "Single yes/no question Aura generates per screen",
...
class FeedbackExperimenter:
    ...
    Feedback mechanisms are themselves A/B tested — Aura learns which
    ...
    async def _design_via_llm(self, hypothesis: str) -> Dict[str, Any]:
        ...
        prompt = f"""You are Aura's experiment designer. A hypothesis has been provided.
...
Return ONLY valid JSON."""
    ...
    async def generate_hypothesis(self) -> str:
        """
        Aura generates her own experiment hypothesis by analyzing:
    ...
    async def _hypothesize_via_llm(self, analytics: Dict[str, Any]) -> str:
        ...
        prompt = f"""You are Aura, an AI that improves human fulfilment.
...
Return ONLY the hypothesis string, no JSON."""
    ...
    async def evaluate_experiment(self, experiment_id: str) -> ExperimentResult:
        ...
        promotes winning mechanism, writes lesson to aura_lessons.
        """
        row = await fetchrow(
    ...
    async def _write_lesson(
        ...
        """Write experiment result as a lesson to aura_lessons."""
