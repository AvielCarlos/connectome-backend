"""
Aura Autonomy Agent — Self-directed system improvement

Aura's autonomous decision loop. Runs on a schedule to:
...
class AuraAutonomyAgent:
    """
    Aura's autonomous improvement engine.
    ...
    async def _watchdog_step(
        ...
        try:
            ...
            try:
                ...
                r = await get_redis()
                await r.delete(f"aura:autonomy:failures:{step_name}")
            except Exception:
                pass
            ...
            try:
                from core.redis_client import get_redis
                r = await get_redis()
                fail_key = f"aura:autonomy:failures:{step_name}"
                fail_count = await r.incr(fail_key)
                await r.expire(fail_key, 7 * 24 * 3600)

                if fail_count >= 5:
                    # Disable this step and alert Avi
                    await r.set(f"aura:autonomy:disabled:{step_name}", "1", ex=48 * 3600)
                    await self._send_telegram(
                        f"⚠️ *Aura Autonomy Step Disabled*\n\n"
    ...
    async def _is_step_disabled(self, step_name: str) -> bool:
        ...
        try:
            from core.redis_client import get_redis
            r = await get_redis()
            return bool(await r.get(f"aura:autonomy:disabled:{step_name}"))
        except Exception:
            return False
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        This is called by POST /api/aura/autonomy/run.
        ...
        if not await self._is_step_disabled("ab_auto_promote"):
            try:
                ...
                try:
                    from core.redis_client import get_redis
                    r = await get_redis()
                    await r.delete("aura:autonomy:failures:ab_auto_promote")
                except Exception:
                    pass
            except Exception as e:
        ...
        if not await self._is_step_disabled("detect_and_fix_bugs"):
            try:
                ...
                try:
                    from core.redis_client import get_redis
                    r = await get_redis()
                    await r.delete("aura:autonomy:failures:detect_and_fix_bugs")
                except Exception:
                    pass
            except Exception as e:
        ...
        if not await self._is_step_disabled("optimize_feed_weights"):
            try:
                ...
                try:
                    from core.redis_client import get_redis
                    r = await get_redis()
                    await r.delete("aura:autonomy:failures:optimize_feed_weights")
                except Exception:
                    pass
            except Exception as e:
        ...
        if not await self._is_step_disabled("self_improvement"):
            try:
                from aura.agents.self_improvement_agent import SelfImprovementAgent
                ...
                try:
                    from core.redis_client import get_redis
                    r = await get_redis()
                    await r.delete("aura:autonomy:failures:self_improvement")
                except Exception:
                    pass
            except Exception as e:
        ...
        if not await self._is_step_disabled("evolution"):
            try:
                from aura.agents.evolution_agent import AuraEvolutionAgent
        ...
        if not await self._is_step_disabled("product_design"):
            try:
                from aura.agents.ux_research_agent import UXResearchAgent
                from aura.agents.ui_evolution_agent import UIEvolutionAgent
                from aura.agents.feature_spawn_agent import FeatureSpawnAgent
        ...
        # L. Survival check — Aura's comprehensive self-healing system
        try:
            from aura.agents.survival_agent import SurvivalAgent
        ...
        try:
            from core.redis_client import get_redis
            r = await get_redis()
            await r.set("aura:autonomy:last_run", json.dumps({
        ...
        for variant in VARIANTS:
            ...
            for e in events:
                ...
                if et == "card_swiped":
                    ...
                    weighted_sum += value * ENGAGEMENT_WEIGHTS["aura_opened"]
    ...
    async def _attempt_auto_fix(self, error_line: str, fix_type: str) -> Dict[str, Any]:
        ...
        try:
            prompt = f"""You are Aura's auto-repair agent. A production error occurred:
...
- If you cannot suggest a safe fix, output {{"file": null, "description": "cannot fix safely"}}"""
            ...
            if patch and target_file:
                await self._send_telegram(
                    f"🔧 *Aura Auto-Fix Suggestion*\n\n"
                    f"Error: `{error_line[:100]}`\n\n"
                    f"File: `{target_file}`\n"
                    f"Fix: {description}\n\n"
                    f"Patch:\n```python\n{patch[:300]}\n```\n\n"
                    f"_(Aura suggests this fix — please review and apply if appropriate)_"
                )
    ...
    async def _optimize_feed_weights(self) -> Dict[str, Any]:
        ...
        r = await get_redis()
        current_weights_raw = await r.get("aura:agent_weights")
        ...
        if changes:
            ...
            # Store in Redis
            await r.set("aura:agent_weights", json.dumps(normalized))
    ...
    async def _log_lesson(self, lesson: str, confidence: float, source: str) -> None:
        ...
        try:
            await db_execute(
                """
                INSERT INTO aura_lessons (lesson, confidence, source)
                VALUES ($1, $2, $3)
                """,
                lesson,
                confidence,
