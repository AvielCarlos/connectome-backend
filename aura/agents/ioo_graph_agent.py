...
from aura.agents.recommendation_engine import (
...
class IOOGraphAgent:
    ...
    async def find_path(
        ...
        for index, node_id in enumerate(path_ids, start=1):
            ...
            if is_goal:
                why = "This is your destination — the outcome Aura is mapping you toward."
            elif index == 1:
            ...
            if not eligibility.get("eligible"):
                for bridge in eligibility.get("bridge_nodes", []):
                    ...
                    personalised.append({
                        ...
                        "why_this_step": "Aura spawned this bridge step because a requirement gap blocks the next node.",
                    })
                    seen.add(bid)
    ...
    async def grow_node_from_angles(
        ...
        for angle in angles[:max_new]:
            ...
            description = (
                ...
                f"Generated as an IOO neural-graph branch so Aura can test which pathway creates more engagement, experiences, activity, and fulfilment."
            )
            row = await fetchrow(
                """
                INSERT INTO ioo_nodes
        ...
        for group in groups:
            ...
            for idx, duplicate in enumerate(duplicates, start=1):
                ...
                description = (
                    ...
                    f"Created from a duplicate IOO node so Aura can compare pathway outcomes instead of carrying redundant graph nodes."
                )
                await execute(
                    """
                    UPDATE ioo_nodes
    ...
    async def run_neural_lifecycle_sweep(self) -> dict:
        ...
        if len(grown) or pruned.get("pruned") or merged.get("merged"):
            try:
                from aura.agents.backup_freshness import trigger_identity_backup
    ...
    async def upsert_world_signal_node(self, signal: dict) -> dict:
        ...
        World-aware Aura should continuously grow the IOO graph from live data:
