"""
TrendSpotter — Keeps Aura culturally current with daily Twitter trends.
...
class TrendSpotter(BaseWorkerAgent):
    ...
    async def run(self) -> None:
        ...
        for insight in insights[:3]:  # top 3 trends
            ...
            await self.teach_aura(
                ...
                f"Consider weaving this into today's content or Aura's coaching conversations.",
                confidence=0.75,
            )

        logger.info(f"TrendSpotter: taught Aura {len(insights[:3])} trends.")
    ...
    async def report(self) -> str:
        return "TrendSpotter: Daily trend scan complete. Aura is culturally up to date."
