"""
SupportAgent — Monitors Aura conversations for user issues.
...
class SupportAgent(BaseWorkerAgent):
    ...
    async def run(self) -> None:
        ...
        data = await self._get("/api/aura/conversations?limit=200&range=4h") or {}
    ...
    async def _handle_issue(self, issue: dict) -> None:
        ...
        # Reply via Aura
        if user_id:
            await self._post("/api/aura/chat", {
                "user_id": user_id,
                "message": (
