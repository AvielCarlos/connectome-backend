"""
LearningAgent — Feeds Aura new knowledge from recent research.

Reports to: Aura (directly)
...
RESEARCH_TOPICS = [
    ...
    },
    {
        ...
        "category": "AI UX",
        "lesson_template": "AI personalization research: {summary}. Applied to Aura: personalization increases engagement by matching content to user context, not just preferences.",
    },
]
...
class LearningAgent(BaseWorkerAgent):
    ...
    async def run(self) -> None:
        ...
        for topic in RESEARCH_TOPICS:
            ...
            if success:
                taught += 1
                logger.info(f"LearningAgent: taught Aura about '{topic['category']}'")

        logger.info(f"LearningAgent: done. Taught Aura {taught}/{len(RESEARCH_TOPICS)} topics.")
    ...
    async def report(self) -> str:
        return "LearningAgent: Weekly knowledge update complete. Aura has been briefed on latest research."
