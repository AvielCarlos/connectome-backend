...
ANALYTICS_DIR = os.path.join(os.getenv("CONNECTOME_RUNTIME_DIR", "/tmp/connectome"), "analytics")
...
class AnalyticsAgent(BaseWorkerAgent):
    ...
    async def run(self) -> None:
        ...
        conv_data = await self._get("/api/aura/conversations?range=7d&limit=100") or {}
        ...
        await self.teach_aura(
            ...
            f"Users who engage deeply with Aura are showing stronger habit formation.",
            confidence=0.85,
        )
