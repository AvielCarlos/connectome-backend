"""
SurfaceRegistry — tracks all Aura-spawned web surfaces.
...
_REDIS_PREFIX = "aura:surfaces"
_REDIS_TTL    = 86_400  # 24 hours


class SurfaceRegistry:
    """Track and retrieve Aura-spawned web surfaces."""
    ...
    async def get_surface(self, surface_id: str) -> Optional[Dict[str, Any]]:
        ...
        row = await fetchrow(
            "SELECT * FROM aura_surfaces WHERE id = $1",
            surface_id,
        )
        if not row:
            return None
    ...
    async def increment_view_count(self, surface_id: str) -> None:
        ...
        try:
            await execute(
                "UPDATE aura_surfaces SET view_count = view_count + 1 WHERE id = $1",
                surface_id,
            )
            await self._cache_del(surface_id)
    ...
    async def retire(self, surface_id: str) -> None:
        ...
        await execute(
            "UPDATE aura_surfaces SET status = 'retired', updated_at = NOW() WHERE id = $1",
            surface_id,
        )
        await self._cache_del(surface_id)
