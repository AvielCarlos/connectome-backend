#!/usr/bin/env python3
"""Fail if live secrets are present in files that can become public.

This is intentionally conservative. Real secrets belong only in managed secret
stores (Railway variables, GitHub Secrets, 1Password, etc.), never in repo files,
docs, screenshots, examples, or agent notes.
"""
...
def scan_file(path: Path, root: Path) -> list[str]:
    ...
    if rel.name in PUBLIC_NOTE_FILES and path.exists():
        # Public repo notes are too easy to leak. Keep local notes outside git.
        issues.append(f"{rel}: public/reference note file must not be tracked or committed")
    if not is_text(path):
        return issues
    try:
...
def main() -> int:
    ...
    if issues:
        ...
        print("\nDo not place live keys, tokens, passwords, or private agent notes in public repo files/docs.", file=sys.stderr)
        return 1
